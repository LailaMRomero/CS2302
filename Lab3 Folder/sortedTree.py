...
#def TreeToArray(node, root):
 #   arr = []
  #  if node is None:
   #     return None
    #TreeToArray(root.leftChild())
  #  arr[index++] = root.item
    #TreeToArray(root.rightChild())
...
def sortedArray(T): 
    ...
    mid = (len(T)) // 2
    root = Node(T[mid]) 
    root.left = sortedArray(T[:mid]) 
    root.right = sortedArray(T[mid+1:]) 
    return root 
def preOrder(node): 
    if not node: 
        return
